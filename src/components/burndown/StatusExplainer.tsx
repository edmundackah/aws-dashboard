...
function StatusItems() {
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-sm leading-6"><span className="font-medium text-primary">Completed</span> — zero remaining achieved by the target date</div>
        </div>
        <div className="flex items-center gap-3">
          <span className="ml-2 inline-block size-2.5 rounded-full bg-[hsl(var(--chart-ms))] dark:bg-[hsl(var(--chart-ms)/0.55)]" />
          <div className="text-sm leading-6"><span className="font-medium text-primary">On track</span> — projected completion (based on burn rate) is before the target date</div>
        </div>
        <div className="flex items-center gap-3">
          <span className="ml-2 inline-block size-2.5 rounded-full bg-amber-400 dark:bg-amber-300/70" />
          <div className="text-sm leading-6"><span className="font-medium text-primary">At risk</span> — projected completion exceeds target date or low confidence in projections</div>
        </div>
        <div className="flex items-center gap-3">
          <span className="ml-2 inline-block size-2.5 rounded-full bg-red-500 dark:bg-red-400/70" />
          <div className="text-sm leading-6"><span className="font-medium text-primary">Target missed</span> — target date passed with items still remaining</div>
        </div>
        <div className="pt-2 text-sm text-primary/80 leading-6">
          <div className="mb-1">Status is calculated using linear regression on historical data with confidence scores.</div>
          <div>Dotted lines show planned remaining; solid lines show actual remaining. Each environment has two targets (SPA and Microservices).</div>
        </div>
      </div>
  )
}
